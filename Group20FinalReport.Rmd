...
```{r}
ggplot(data = MetaData) + geom_bar(aes(x = runtime, fill = adult)) + xlim(0,200)
```
From the MetaData table alone we see that there are almost no cases where the film is classified as an adult film, and the data in turms of runtime presents itself in the shape of a bell-curve. 

From the research conducted in this project, we have concluded a few things about what components are popular in movies and produce different outcomes in terms of popularity. Many of the films we had in our data tended to be non-adult rated films. Additionally, the runtime for movies were 90 minutes or longer, and had a sizeable budget for better quality and production. Drama was one of the most popular genres included in our table, meaning it is one of the most popular genres or subgenres that a movie category falls into. Looking more into actors, producers, and collection company proved to be a challenge as well as looking into other larger components from both tables that were present in one and not the other. Doing so would produce skewed data and innaccurate results. Another challenge was trying to use the measure of popularity from the MetaData table because though it supports certain claims accurately, since there were no matching titles from the two data tables, it would've fallen into the same trap as the others did with outputting skewed results. We haven't been able to confidently figure out how the algorithm for the streaming sites work in terms of what movies and shows are selected to be suggested to users, but we were successful in identifying key aspects that are popular in many films. Movies that end up being suggested to users should have at least one of the criterion listed whether its the length of the film or the genre. Other factors could and should be considered too, not just those. Popular actors can make a film just as likely to be suggested as another, we just couldn't figure out how to make that work with our research. 